...
class Exercise:
    ...
    def to_image_urls(
        self,
        base_url: str = "https://raw.githubusercontent.com/yuhonas/free-exercise-db/main/exercises/",
    ) -> List[str]:
        ...
        try:
            ...
            if check_duplicates:
                ...
                if duplicates:
                    ...
                    logger.info(
                        f"Found {len(duplicates)} potential duplicate(s) for '{exercise.name}'"
                    )
...
""")
            ...
            for i, exercise in enumerate(exercises, 1):
                ...
                if not dry_run:
                    imported = self.import_exercise(
                        conn, exercise, check_duplicates=not skip_duplicate_check
                    )
    ...
    def _print_summary(self, dry_run: bool):
        ...
        logger.info(
            f"New exercises {'to be imported' if dry_run else 'imported'}: {self.stats['new_exercises']}"
        )
...
def main():
    ...
    importer = ExerciseImporter(
        db_url=args.db_url, exercises_file=args.exercises_file, output_dir=args.output_dir
    )
