...
select -- fe.base_exercise_id,
       ...
       coalesce(max(pe.weight), 0) + (case when pe.successful then 2500 else 0 end) as weight
...
group by fe.base_exercise_id, fe.name, fe.exercise_id, ps.performed_session_id, fe.reps, fe.sets, fe.rest, pe.successful
