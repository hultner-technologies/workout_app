-- Improved session interface with JSON aggregation
--
-- Problem with v2: Session metadata repeated in every row, client must aggregate
-- Solution: Return single row with session info + aggregated exercises array
--
-- Benefits:
-- - Clean JSON structure (single object, not array of rows)
-- - Session metadata at top level
-- - Exercises as nested array
-- - Always returns 1 row (empty array for no exercises)
-- - Easier to consume in APIs
--
-- Created: 2025-11-06
-- Author: Claude (Anthropic)

-- Function: draft_session_exercises_v3
...
CREATE OR REPLACE FUNCTION draft_session_exercises_v3(performed_session_id_ uuid)
    RETURNS TABLE (
...
exercise_data AS (
    ...
    WHERE si.performed_session_id = performed_session_id_
        AND fe.exercise_id IS NOT NULL  -- Only include actual exercises
    GROUP BY
        fe.base_exercise_id,
        fe.name,
        fe.exercise_id,
...
COMMENT ON FUNCTION draft_session_exercises_v3(uuid) IS
    'Returns session metadata with exercises aggregated into a JSON array. '
...
-- Function: performed_session_details_v2
-- Improved version that returns session info with JSON aggregated exercises
DROP FUNCTION IF EXISTS performed_session_details_v2(uuid);

CREATE OR REPLACE FUNCTION performed_session_details_v2(performed_session_id_ uuid)
    RETURNS TABLE (
        exists boolean,
        performed_session_id uuid,
        session_schedule_id uuid,
...
COMMENT ON FUNCTION performed_session_details_v2(uuid) IS
    'Enhanced version with timestamps. Check if a performed session exists and get its metadata. '
